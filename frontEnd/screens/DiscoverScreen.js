...
class DiscoverScreen extends React.Component {
	...
	orderByAvailability = (ascending) => {
		this.setState((currentState) => {
			const itemsClone = [ ...currentState.items ];
			const sortedItems = itemsClone.sort((a, b) => {
				return ascending ? a.isAvailable - b.isAvailable : b.isAvailable - a.isAvailable;
			});
			return { items: sortedItems };
		});
	};
	...
	render () {
		return (
			<View style={styles.container}>
				...
				/>
				<CategorySelector sortByCategory={this.sortByCategory} resetResults={this.resetResults} />
				<ScrollView>
					...
					{this.state.bottomBorder && <View style={styles.bottomPadding} />}
				</ScrollView>
			</View>
		);
	}
